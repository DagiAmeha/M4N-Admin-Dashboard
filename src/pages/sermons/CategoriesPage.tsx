  ...
  return (
    <div>
      <PageHeader
        ...
        action={<Button onClick={openCreate} variant="light"><Plus size={16} />New Category</Button>}
      />
      ...
      <Table
        ...
        columns={[
          ...
          { header: 'Description', cell: (c) => <span className="text-gray-500 truncate max-w-xs block">{c.description ?? '—'}</span> },
          {
            header: 'Actions',
            cell: (c) => (
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Category' : 'New Category'}>
        <div className="space-y-4">
          <Input label="Name *" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Faith & Prayer" />
          ...
          <FileUpload
            ...
            current={imageFile?.name ?? (editing?.imageUrl ? 'Current image' : undefined)}
          />
